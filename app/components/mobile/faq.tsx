import { FAQItemMobile, IFAQItem } from '../sub-components/faqItem';
import FAQBackgroundMobile from '../../../public/icons/FAQBackgroundMobile.svg';
...
export default function FAQMobile() {
  return (
    <div className="relative pt-[35.641rem]">
      <div className="relative w-full flex justify-center faq-header z-20">
        <span className="text-[14.872rem] leading-[110%]">FAQ</span>
      </div>

      <figure className="absolute top-[-30rem] z-10">
        <img src={FAQBackgroundMobile.src} className="w-[130rem] h-[177.436rem]" />
      </figure>

      <div className="relative pt-[20.513rem] px-[5.128rem] z-30">
        <div className="">
          {dataItems.map((item, index) => {
            return <FAQItemMobile key={index} title={item.title} description={item.description} />;
          })}
        </div>
      </div>
    </div>
